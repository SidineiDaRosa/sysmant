    ...
    function drawChart() {
        ...
        table = document.getElementById("minhaTabela");
         ...
         for (var i = 1; i < table.rows.length; i++) {
            let codId = document.getElementById("minhaTabela").rows[i].cells[0].innerHTML;
            let patrimonio = document.getElementById("minhaTabela").rows[i].cells[7].innerHTML;
            let nome = document.getElementById("minhaTabela").rows[i].cells[9].innerHTML;
            let executar = document.getElementById("minhaTabela").rows[i].cells[10].innerHTML;
            let executado = document.getElementById("minhaTabela").rows[i].cells[11].innerHTML;
            let nomeExec='Res:'+nome+', Desc:'+executar+"->"+executado
            
        //data inicial
        let dataInic = document.getElementById("minhaTabela").rows[i].cells[3].innerHTML;
        //hora inicial
        let horaInic = document.getElementById("minhaTabela").rows[i].cells[4].innerHTML;

        //data final
        let dataFim = document.getElementById("minhaTabela").rows[i].cells[5].innerHTML;
        //hora final
        let horaFim = document.getElementById("minhaTabela").rows[i].cells[6].innerHTML;
       ...
       let horaAmericanaInic = (dataTimeInic.toLocaleTimeString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true }))
      ...
      let horaFin= dataTimeFim.getHours()
       ...
       // alert('segundos:' + segundos)
        let horaAmericanaFim = (dataTimeFim.toLocaleTimeString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true }))
       // alert(horaAmericanaFim)
        //Gera linhas time line
        let dataTimeInicio=anoInic+'/0'+mesInic+'/'+diaInic+','+horaInicial+':'+minutosInic+':'+segundosInic;
        
        //alert(dataTimeInicio)
        let dataTimeFinal=anoFin+'/0'+mesFin+'/'+diaFin+','+horaFin+':'+minFin+':'+segFin
        
       // alert(dataTimeFinal)
        dataTable.addRows([
            ["ID:"+codId+","+patrimonio,nomeExec,new Date(dataTimeInicio), new Date(dataTimeFinal)]]);
        chart.draw(dataTable);
        //renderização de cores do timeline
        var options = {
            colors: ['#78f', '#5af', '#6af'],
            timeline: { rowLabelStyle: {fontName: 'Arial', fontSize: 17, color: '#000000' },
